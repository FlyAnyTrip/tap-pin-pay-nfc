...
const NFCReaderComponent = ({ isActive = true, onProductAdded }) => {
  ...
  const checkNFCSupport = () => {
    if ("NDEFReader" in window) {
      ...
      showSuccess("📱 NFC is supported on this device")
    } else {
      setIsNFCSupported(false)
      console.log("❌ Web NFC API is not supported")
      showError("NFC is not supported on this device or browser")
    }
  }
  ...
  const requestNFCPermission = async () => {
    try {
      ...
      if (permission.state === "granted") {
        ...
        showError("NFC permission denied. Please enable NFC permissions.")
        return false
      }
    } catch (error) {
  ...
  const startNFCReading = async () => {
    ...
    try {
      ...
      const loadingId = showLoading("Requesting NFC permissions...")
      ...
      setPermissionGranted(true)
      removeNotification(loadingId)
      showSuccess("🎯 NFC Reader Active - Tap an NFC tag")
    } catch (error) {
      ...
      if (error.name === "NotAllowedError") {
        showError("❌ NFC access denied. Please allow NFC permissions.")
      } else if (error.name === "NotSupportedError") {
        showError("❌ NFC is not supported on this device.")
        setIsNFCSupported(false)
      } else if (error.name === "NotReadableError") {
        showError("❌ NFC is disabled. Please enable NFC in device settings.")
      } else {
        showError(`❌ NFC Error: ${error.message || "Unknown error occurred"}`)
      }
    }
  }
  ...
  const handleNFCError = (error) => {
    ...
    setNfcStatus("error")
    showError("❌ Error reading NFC tag. Please try again.")
  ...
  const handleNFCRead = async (message, serialNumber) => {
    try {
      ...
      if (!productId) {
        ...
        setNfcStatus("error")
        showError("❌ Invalid NFC tag - No product information found")
      ...
      const loadingId = showLoading(`🔍 Processing ${productId}...`)
      ...
      if (product) {
        ...
        if (isItemInCart(product.id)) {
          setTimeout(() => {
            setNfcStatus("error")
            removeNotification(loadingId)
            showError(`⚠️ ${product.name} is already in your cart!`)
          ...
          setTimeout(() => {
            ...
            addItemOnce(product)
            removeNotification(loadingId)
            showSuccess(`✅ Added ${product.name} to cart via NFC!`)
        ...
        setTimeout(() => {
          setNfcStatus("error")
          removeNotification(loadingId)
          showError(`❌ Product ${productId} not found in database`)
      ...
      setNfcStatus("error")
      showError("❌ Error processing NFC tag. Please try again.")
